...
public class TheScenes extends NeveraDullMoment {
	...
	public static Player theReport(Scanner in, Player SelectedEmployee) { // This is the method declaration taking in the scanner object and employee object in the parameters
	...
	public static int futurePosition(Scanner in, Player SelectedEmployee) {
		...
		if (total >= 0 && total <= 23) { //31-53
			System.out.println(" You need to find other employment."+ total);

		} else if (total >= 24 && total <= 44) { //54-76
			System.out.println(" To demote you." + total);

		} else  if  (total >= 45 && total <= 65) { // 77-100
			System.out.println(" to put on probation." + total);

		}else {
			System.out.println(" To promote you. Congratulations!!");// anything over 85
		}		
